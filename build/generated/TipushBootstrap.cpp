...
static Handle<Value> Tipush_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tipush.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TipushBindings::lookupGeneratedInit(
...
static void Tipush_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tipush_getBinding)->GetFunction());
}

static void Tipush_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TipushBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TipushBinding = {
	"ti.push",
	Tipush_init,
	Tipush_dispose
};
...
Java_ti_push_TipushBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.push", &TipushBinding);
	titanium::KrollBindings::addExternalLookup(&(::TipushBindings::lookupGeneratedInit));
}
